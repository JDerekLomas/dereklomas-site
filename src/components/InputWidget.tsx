...
export default function InputWidget({
  ...
  useEffect(() => {
    ...
    const visible = hasInputParam || hostAllowed;
  ...
  const loadFeedback = async () => {
    try {
      if (isLocalhost) {
        ...
        // Use localStorage on production
        const stored = localStorage.getItem("getinput-feedback");
        if (stored) {
  ...
  const saveInput = async (item: InputItem) => {
    const newItems = [...feedbackItems, item];
    if (isLocalhost) {
      ...
      // Use localStorage on production
      localStorage.setItem("getinput-feedback", JSON.stringify(newItems));
    }
    setFeedbackItems(newItems);
    setInputCount(newItems.length);
  ...
  const clearFeedback = () => {
    if (isLocalhost) {
      ...
      localStorage.removeItem("getinput-feedback");
    }
    setFeedbackItems([]);
    setInputCount(0);
    showToastWithMessage("Cleared!");
    setMode("idle");
  };
  ...
  return (
    <div
      id="input-widget"
      className="fixed bottom-4 right-4 z-[9999] font-sans print:hidden"
    >
      {showToast && (
            ...
            <>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 flex items-center justify-center bg-red-100 hover:bg-red-200 text-red-600 rounded-lg py-2 text-sm font-medium transition"
              ...
              <p className="text-[10px] text-gray-400 text-center mt-2">
                Copy and paste the JSON into Claude Code
              </p>
            </>
          )}
        </div>
